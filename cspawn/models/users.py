...
class Class(db.Model):
    ...
    recurrence_rule = Column(String(255), nullable=True)  # Recurrence rule (e.g., iCalendar format)
    container_image = Column(String(255), nullable=False)
    start_script = Column(Text, nullable=False)

    instructors = relationship('Instructor', secondary='class_instructors', back_populates='classes')
...
class Instructor(Role):
    ...
    id = Column(UUID(as_uuid=True), ForeignKey('roles.id'), primary_key=True)
    classes = relationship('Class', secondary='class_instructors', back_populates='instructors')
